...
@export var texture_size_x = 32
@export var texture_size_y = 20
...
func _ready() -> void:
	$Camera2D.limit_left = texture_size_x/2
	$Camera2D.limit_right = width * texture_size_x
	$Camera2D.limit_top = texture_size_y/2
	$Camera2D.limit_bottom = height * texture_size_y / 2
	if seed == -1:
		seed = random.randi()
		#print(seed)
	...
	for y in height:
		for x in width:
			...
			$TileMapLayer.set_cell(Vector2i(x,y),1,Vector2i(0,noise_value_processed))
